...
def send_message(msg_info):
    ...
    send = subprocess.run("echo '{}' | netcat -c -w 2 {} 12345"
                          .format(msg_obj_srl, recipient_profile["ip"]), shell=True)
...
def local_discover():
    ...
    for ip in range(0, int(my_ip)):
        subprocess.Popen("echo '{}' | netcat -c -w 1 192.168.1.{} 12345".format(disc_obj_srl, ip),
                         shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    for ip in range(int(my_ip)+1, 254):
        subprocess.Popen("echo '{}' | netcat -c -w 1 192.168.1.{} 12345".format(disc_obj_srl, ip),
                         shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def hamachi_discover(ip_list):
    ...
    for ip in ip_list:
        subprocess.Popen("echo '{}' | netcat -c -w 1 {} 12345"
                         .format(hamachi_obj_srl, ip), shell=True)
    print("{} Hamachi client discovery completed!".format(emoji.emojize(":mushroom:")))


def discovery_actions(msg):
    global chat_dict, profile_dict
    ...
    subprocess.Popen("echo '{}' | netcat -c {} 12345".format(rsp_obj_srl, msg["MY_IP"]),
                     shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def listener_func(writer):
    global isExit, listenPid
    while not isExit:
        listen = subprocess.Popen("netcat -l -p 12345", shell=True, stdout=writer, stderr=subprocess.DEVNULL)
        listenPid = listen.pid
        listen.communicate()


def message_parse(file):
    global isNotified
    raw_messages = file.readlines()
    raw_messages = [msg.strip() for msg in raw_messages if msg != "\n" and msg != "\r"]
...
def messenger_func(reader):
    while True:
        if isExit:
            break
        message_parse(reader)
...
if __name__ == "__main__":
    write = open("./messages.txt", "w")
    read = open("./messages.txt", "r")
    entry()
    env_params("local")
    hamachi_ip_list = hamachi_ip_extract()
    listen_func = threading.Thread(target=listener_func, args=(write,))
    listen_func.daemon = True
    listen_func.start()
    message_func = threading.Thread(target=messenger_func, args=(read,))
    ...
    subprocess.run("kill -9 {}".format(listenPid), shell=True)
    message_func.join()
    listen_func.join()
    read.close()
    write.close()
